...
public class CreateRepositoryProgressPageUserControl : BasePageUserControl
{
	...
	private List<Instruction> CreateRepositoryInstructions => repositoryOperation.Instructions;

	private List<Instruction> CreateDatabaseInstructions => repositoryOperation.CreateDatabaseInstructions;

	private string ConnectionString => repositoryOperation.ConnectionString;

	private string Database => repositoryOperation.Database;

	public CreateRepositoryProgressPageUserControl()
	{
		InitializeComponent();
	...
	private void BackgroundWorker_DoWork(object sender, DoWorkEventArgs e)
	{
		...
		bool? flag = CreateDatabase(e);
		if (flag != false)
		{
			bool wasDatabaseJustCreated = flag == true;
			CreateRepositoryTables(e, wasDatabaseJustCreated);
		}
	}

	private bool? CreateDatabase(DoWorkEventArgs e)
	{
		if (CreateDatabaseInstructions == null)
		{
			return null;
		}
		using (SqlConnection sqlConnection = new SqlConnection(ConnectionString))
		{
			...
			{
				TrackingService.MakeAsyncRequest(new ParametersWithUserDataedoRepoBuilder(new TrackingConnectToRepositoryRepoParameters(isFile: false), new TrackingDataedoParameters(), new TrackingUserParameters()), TrackingEventEnum.CreatorNewServerConnected);
			});
			int count = CreateDatabaseInstructions.Count;
			for (int i = 0; i < count; i++)
			{
				Instruction instruction = CreateDatabaseInstructions[i];
	...
	private void CreateRepositoryTables(DoWorkEventArgs e, bool wasDatabaseJustCreated)
	{
		...
		int count = CreateRepositoryInstructions.Count;
		for (int i = 0; i < count; i++)
		{
			Instruction instruction = CreateRepositoryInstructions[i];
			if (backgroundWorker.CancellationPending)
			{
				e.Cancel = true;
				break;
			...
			int num = (int)Math.Round(((float)i + 1f) / (float)CreateRepositoryInstructions.Count * 100f);
	...
	private void BackgroundWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
	{
		...
		string title;
		string description;
		if (exceptions.Length == 0 && !e.Cancelled && e.Error == null)
		{
			title = "Repository created";
			description = "Repository has been successfully created.";
			...
			StaticData.SetRepositoryData(RepositoriesDBHelper.GetRepositoryData(repositoryOperation.Database, isProjectFile: false, repositoryOperation.DatabaseConnectionString));
			TrackingRunner.Track(delegate
			{
				string time = (DateTime.UtcNow - startTime).TotalSeconds.ToString();
				TrackingService.MakeAsyncRequest(new ParametersWithUserDataedoRepoBuilderWithEventSpecificTime(new TrackingRepoParameters(), new TrackingDataedoParameters(), new TrackingUserParameters(), time), TrackingEventEnum.CreatorNewServerCreated);
			});
			return;
		}
		if (exceptions.Length > 0)
		{
			...
			description = "See following log for details.";
		}
		else if (e.Cancelled)
		{
			title = "Repository not created";
	...
	private void InitializeComponent()
	{
		...
		this.contentPanelControl.Size = new System.Drawing.Size(646, 273);
		...
		this.description1LabelControl.Text = "Please wait while the repository is being created.";
		...
		this.headerLabelControl.Text = "Creating repository";
		...
		this.logoPictureEditTopSeparatorEmptySpaceItem.Location = new System.Drawing.Point(0, 378);
		...
		this.logoPictureEditTopSeparatorEmptySpaceItem.Size = new System.Drawing.Size(97, 14);
		...
		this.logoPictureEditRightSeparatorEmptySpaceItem.Location = new System.Drawing.Point(97, 378);
		this.logoPictureEditRightSeparatorEmptySpaceItem.Name = "logoPictureEditRightSeparatorEmptySpaceItem";
		this.logoPictureEditRightSeparatorEmptySpaceItem.Size = new System.Drawing.Size(458, 42);
		...
		this.buttonsEmptySpaceItem.Location = new System.Drawing.Point(555, 378);
		this.buttonsEmptySpaceItem.Name = "buttonsEmptySpaceItem";
		this.buttonsEmptySpaceItem.Size = new System.Drawing.Size(95, 13);
		...
		this.contentPanelControlLayoutControlItem.Size = new System.Drawing.Size(650, 277);
		...
		base.Name = "CreateRepositoryProgressPageUserControl";
