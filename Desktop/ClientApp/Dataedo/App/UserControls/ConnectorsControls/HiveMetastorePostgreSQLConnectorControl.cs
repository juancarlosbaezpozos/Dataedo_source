...
public class HiveMetastorePostgreSQLConnectorControl : PostgreSQLConnectorControl
{
	private ButtonEdit hiveDatabaseButtonEdit;

	private LayoutControlItem hiveDatabaseLayoutControlItem;

	protected override PanelTypeEnum.PanelType PanelType => PanelTypeEnum.PanelType.HiveMetastorePostgreSQL;
	...
	public HiveMetastorePostgreSQLConnectorControl()
	{
	...
	protected override void SetPanelNewDBRowValues(bool forGettingDatabasesList = false)
	{
		...
		base.DatabaseRow = new HiveDatabaseRow(base.SelectedDatabaseType, (!forGettingDatabasesList) ? postgreSqlDatabaseButtonEdit.Text : base.DatabaseRow?.Param2, SelectedHiveDatabase?.CatalogName, SelectedHiveDatabase?.DatabaseName, documentationTitle, base.providedPostgreSqlHost, postgreSqlLoginTextEdit.Text, postgreSqlPasswordTextEdit.Text, base.providedPostgresPort, base.DatabaseRow.Id, base.DatabaseRow.Filter.GetRulesXml(), base.DatabaseRow.DbmsVersion, postgreSqlSSLModeLookUpEdit.EditValue.ToString());
	}

	public override void SetParameters(int? databaseId = null, bool? isCopyingConnection = false, bool isExporting = false)
	{
		postgreSqlDatabaseLayoutControlItem.Text = "Metastore database:";
	...
	protected override void ReadPanelValues()
	{
		...
		postgreSqlDatabaseButtonEdit.Text = base.DatabaseRow.Param2;
	...
	private void SetHiveDatabaseControlPosition()
	{
		...
		postgresLayoutControl.Root.Remove(hiveDatabaseLayoutControlItem);
		if (base.SelectedDatabaseType.HasValue)
		{
			postgresLayoutControl.Root.AddItem(hiveDatabaseLayoutControlItem, postgreSqlDatabaseLayoutControlItem, InsertType.Bottom);
		}
	}
	...
	private void hiveDatabaseButtonEdit_ButtonClick(object sender, ButtonPressedEventArgs e)
	{
		...
		SetNewDBRowValues();
		if (!HiveDatabaseRow.ValidateHiveDatabase(addDBErrorProvider, postgreSqlDatabaseButtonEdit, base.SplashScreenManager, FindForm()))
		{
