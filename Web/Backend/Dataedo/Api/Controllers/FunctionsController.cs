...
[Authorize]
[ApiController]
public class FunctionsController : BaseObjectController, IDependencies
{
	...
	/// </summary>
	protected new IFunctionsService Service => base.Service as IFunctionsService;
	...
	public FunctionsController(IRepositoryAccessManager repositoryAccessManager)
		: base(repositoryAccessManager.GetRepository().FunctionsService, repositoryAccessManager.GetRepository().FunctionsWriteService)
	{
	...
	public virtual async Task<IActionResult> Get([Range(0, int.MaxValue)] int id, [FromQuery] FunctionsJoinsEnum.FunctionsJoins[] join = null)
	{
		return Ok(await Service.GetObject(id, EnumTools.Convert<FunctionsJoinsEnum.FunctionsJoins, ProceduresFunctionsJoinsEnum.ProceduresFunctionsJoins>(join)));
	}
