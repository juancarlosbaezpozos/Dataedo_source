...
public class ProcedureChangeController : ControllerBase
{
	private readonly IObjectChangeDetailsService objectDetailsService;

	private readonly ISCTCommandsService editService;

	public ProcedureChangeController(IRepositoryAccessManager repositoryAccessManager)
	{
	...
	[ServiceFilter(typeof(ProcedureChangesViewAttribute))]
	[HttpGet("{id}/details")]
	public virtual async Task<IActionResult> GetObjectChangeDetails([Range(0, int.MaxValue)] int id)
	{
		return Ok(await objectDetailsService.GetProcedureChangeDetails(id));
	}

	[ServiceFilter(typeof(ProcedureChangeDocumentationEditAttribute))]
	...
	{
		return Ok(await editService.UpdateDescription<ProceduresChanges>(descriptionUpdateDTO));
	}
}
