DECLARE @id AS UNIQUEIDENTIFIER = '82f9baf9-7bff-4f65-8b4a-3213d1909de3'
DECLARE @version as int = 10
DECLARE @update as int = 3
DECLARE @release as int = 0
DECLARE @change_no as varchar(25) = 'DEV-3467'
...
IF (SELECT COUNT(*) FROM [version] WHERE [version] = @version AND [update] = @update AND [release] = @release) = 0
  ...
  /*-----------------------------------------------------------------------------------------------------------------------------*/
    ALTER TABLE [modules] ADD [erd_show_nullable] [bit] NOT NULL DEFAULT 0;
